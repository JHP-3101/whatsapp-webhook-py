...
class SessionManager:
    ...
    def _cleanup_sessions(self):
        while True:
            ...
            for user, session in list(self.user_sessions.items()):
                if (
                    session.get("active") 
                    and not session.get("ended") 
                    and now - session["last_active"] > timedelta(minutes=1)
                ):
                    session["active"] = False
                    session["ended"] = True
                    logger.info(f"🔴 Session marked as ended for {user}")
            time.sleep(10)
    ...
    def has_session_ended(self, user_id):
        return self.user_sessions.get(user_id, {}).get("ended", False)
